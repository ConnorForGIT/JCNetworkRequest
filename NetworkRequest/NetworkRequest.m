...
+ (TheURLSessionTask *)_requestWithUrl:(NSString *)url
...
+ (TheURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    TheURLSessionTask *session = [manager uploadTaskWithRequest:request fromFile:[NSURL URLWithString:uploadingFile] progress:^(NSProgress * _Nonnull uploadProgress) {
        if (progress) {
...
+ (TheURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    AFHTTPSessionManager *manager = [self manager];
    TheURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (TheURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    TheURLSessionTask *session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        if (progressBlock) {
...
+ (NSString *)theURLEncode:(NSString *)url {
    NSString *newString =
